...
export const relatoriosService = {
  async findOptions() {
    const response = await api.get<RelatoriosOptionsResponse>('/relatorios/options')
  ...
  async downloadAuditoriaMovimentacoes(
    ...
    const response = await api.get<Blob>('/relatorios/auditoria-movimentacoes', {
      params: filters,
      responseType: 'blob',
    })
  ...
  async downloadHistoricoPatrimonio(patrimonioId: string) {
    const response = await api.get<Blob>(`/relatorios/patrimonios/${patrimonioId}/historico`, {
      responseType: 'blob',
    })
